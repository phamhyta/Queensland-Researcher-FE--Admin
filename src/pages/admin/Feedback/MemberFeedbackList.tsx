...
import { getAllFeedback } from "../../../utils/api/feedback";

const MemberFeedbackList = () => {
    const { id } = useParams();
    ...
    return (
        <div>
            <h1 className='text-center text-2xl font-bold mb-10'>List of information received</h1>
						...
						{feedbacks.length > 0 &&
							feedbacks?.map((feedback) => (
								<tr key={feedback.id} className='odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'>
									...
									<td className='px-6 py-4'>
										<div onClick={() => navigate(`/admin/members/${feedback.user_id}`)} className="cursor-pointer"> {feedback.username} </div>
									</td>
...
export default MemberFeedbackList;
